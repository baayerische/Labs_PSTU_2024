...
void add_key_at_position(TMap& m, double key, int pos) {
    TMap temp;
    int index = 0;
    auto it = m.begin();
...
void remove_key(TMap& m, double key) {
...
double compute_max_min_diff(const TMap& m) {
    if (m.empty()) return 0.0;
    double max_val = m.begin()->second;
    double min_val = m.begin()->second;
    for (const auto& pair : m) {
...
void subtract_diff(TMap& m) {
    double diff = compute_max_min_diff(m);
    TMap temp;
    int index = 0;
    for (auto& pair : m) {
